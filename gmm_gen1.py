...
def choose_k_gmm( scaled_features: np.ndarray, k_range: range, sample_size: int = 20000 ) -> tuple:
...
def run_clustering( df: pd.DataFrame, depth: int ) -> tuple:
    # Runs one round of GMM clustering
    ...
    print("\nSearching for best n_components via BIC...")
    best_k, scores = choose_k_gmm(scaled, K_RANGE)
    print(f"\nBest n_components = {best_k} (BIC={scores[best_k]:.1f})\n")

    print(f"Fitting final GaussianMixture with n_components={best_k} on full dataset...")
    gmm = GaussianMixture(
        n_components=best_k,
        covariance_type=COVARIANCE_TYPE,
        random_state=RANDOM_STATE
    )
    df["cluster"] = gmm.fit_predict(scaled)
