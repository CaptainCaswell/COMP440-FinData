...
BATCH_DELAY_SECONDS = 3 # Delay between batches to avoid rate limiting
...
def download_batch( batch: List[str] ) -> Optional[tuple[pd.DataFrame, pd.DataFrame]]:
    ...
    try:
        # Download batch
        full_df = yf.download( batch, start=START_DATE, end=END_DATE, auto_adjust=True, progress=False )
...
def main():
    ...
    print( f" Getting recent data ({START_DATE.date()} to {END_DATE.date()}) at {start_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
